...
class Camera1:
    ...
    def __init__(self):
        ...
        self.not_detected_pub = rospy.Publisher("feedback_bool", Bool, queue_size=10)
    ...
    def callback(self, data):
        ...
        if len(bluecnts) > 0:
            # Use the contour area to determine if it's a valid detection
            black_area = max(bluecnts, key=cv2.contourArea)
            area_threshold = 100  # Adjust the area threshold as needed
            if cv2.contourArea(black_area) > area_threshold:
                ...
                rospy.loginfo("false")
                self.not_detected_pub.publish(False)
        else:
            # Publish a Bool message indicating that black color is not detected
            rospy.loginfo("false")
            self.not_detected_pub.publish(False)
